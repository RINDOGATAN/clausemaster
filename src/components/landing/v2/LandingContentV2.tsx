...
import { ArrowRight, Check, Loader2, FileText, ClipboardCheck, Shield } from "lucide-react";
...
export default function LandingContentV2({
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ok) setCardMode("sent");
      else setError("Email sign-in is not available. Please use Google.");
    } catch {
  ...
  const handleResendMagicLink = async () => {
    ...
    try {
      const ok = await sendMagicLink(emailInput);
      if (!ok) setError("Failed to resend link. Please try again.");
    } catch {
  ...
  return (
    <main>
      ...
      {/* ═══ WHAT ARE AGENTIC ATTORNEY SKILLS ═══ */}
