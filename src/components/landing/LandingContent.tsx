...
export default function LandingContent({
  ...
  // Detect which email-type provider is registered
  useEffect(() => {
  ...
  const stats = [1, 2, 3, 4].map((n) => ({
    value: t(`revenue.stat${n}.value`),
    label: t(`revenue.stat${n}.label`),
  }));
  ...
  return (
    <main>
      ...
      <section className="py-20 md:py-28">
        <div className="container px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {valueProps.map((vp, i) => (
              <Reveal key={vp.title} delay={i}>
                <div className="paper-card h-full">
        ...
        <div className="container px-6">
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <h2 className="text-2xl md:text-4xl mb-6">
              {t("skills.heading.prefix")}
              <span className="text-accent">{t("skills.heading.accent")}</span>
              {t("skills.heading.suffix")}
            </h2>
            <Reveal>
              <p className="text-base text-muted-foreground leading-relaxed font-body max-w-2xl mx-auto">
                {t("skills.intro")}
              </p>
            </Reveal>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {skillTypes.map((st, i) => (
              <Reveal key={st.title} delay={i}>
                <div className="paper-card h-full flex flex-col">
                  <div className="w-12 h-12 bg-accent/10 rounded-xl flex items-center justify-center mb-4">
                    <st.icon className="w-6 h-6 text-accent" />
                  </div>
                  <h3 className="text-lg font-display mb-2">{st.title}</h3>
                  <p className="text-sm text-muted-foreground leading-relaxed font-body mb-4 flex-1">
                    {st.desc}
                  </p>
                  <p className="text-xs text-accent/80 font-body font-medium">{st.examples}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="container px-6">
          ...
          <div className="max-w-3xl mx-auto">
            {workflowSteps.map((step, i) => (
              <Reveal key={step.title} delay={i} className="mb-6">
      ...
      <section className="py-20 md:py-28 bg-secondary/20 border-y border-border">
        <div className="container px-6">
          <div className="max-w-3xl mx-auto text-center mb-12">
            <span className="section-label">{t("revenue.label")}</span>
            <h2 className="text-2xl md:text-4xl mb-6">
              {t("revenue.heading.prefix")}
              <span className="text-accent">{t("revenue.heading.accent")}</span>
              {t("revenue.heading.suffix")}
            </h2>
            <Reveal>
              <p className="text-base text-muted-foreground leading-relaxed font-body max-w-2xl mx-auto">
                {t("revenue.desc")}
            ...
            {stats.map((stat, i) => (
              <Reveal key={stat.label} delay={i}>
                <div className="stat-card text-center">
                  <div className="metric-lg text-accent mb-2">{stat.value}</div>
        ...
        <div className="container px-6">
          ...
          <div className="space-y-8 max-w-5xl mx-auto">
            {features.map((feature, index) => (
              <Reveal key={feature.id} delay={0}>
                <div className="paper-card group hover:border-accent/50 transition-all duration-300">
      ...
      <section className="py-20 md:py-28 bg-secondary/20 border-y border-border">
        <div className="container px-6">
          <div className="max-w-3xl mx-auto text-center mb-12">
            <span className="section-label">{t("types.label")}</span>
            <h2 className="text-2xl md:text-4xl mb-4">
              {t("types.heading.prefix")}
              <span className="text-accent">{t("types.heading.accent")}</span>
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {/* Contract types */}
            <Reveal delay={0}>
              <div className="paper-card h-full">
                ...
                <h3 className="text-lg font-display mb-4">{t("types.contract.label")}</h3>
                <div className="flex flex-wrap gap-2">
                  {t("types.contract.items").split(",").map((item) => (
                    <span key={item} className="tag text-xs">{item.trim()}</span>
                  ))}
                </div>
              </div>
            </Reveal>
            ...
            <Reveal delay={1}>
              <div className="paper-card h-full">
                ...
                <h3 className="text-lg font-display mb-4">{t("types.assessment.label")}</h3>
                <div className="flex flex-wrap gap-2">
                  {t("types.assessment.items").split(",").map((item) => (
                    <span key={item} className="tag text-xs">{item.trim()}</span>
                  ))}
                </div>
              </div>
            ...
            <Reveal delay={2}>
              <div className="paper-card h-full">
                ...
                <h3 className="text-lg font-display mb-4">{t("types.solo.label")}</h3>
                <div className="flex flex-wrap gap-2">
                  {t("types.solo.items").split(",").map((item) => (
                    <span key={item} className="tag text-xs">{item.trim()}</span>
                  ))}
                </div>
              </div>
