...
node scripts/count-accounts.mjs [--out=DIR]            # Read-only account and activity counts as one JSON object (needs COUNT_DATABASE_URL; owner's go-ahead for production)
```
